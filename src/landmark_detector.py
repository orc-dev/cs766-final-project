"""
File Name: landmark_detector.py
Date Created: Mar.20 2024
Python Version: 3.11

Description:
`video_list` contains the file name in the `data` directory. The `main()` 
function selects one video file to process by generating a new video file
with the labeled landmarks and a `_raw.csv` file, which stores the corrdinates
of these landmarks and a timestamp of each frame.
...
- Run `python3.11 src/landmark_detector.py`
"""
import time
import re
import os
